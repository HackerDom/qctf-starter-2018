...
TITLE = "Эксперты считают интернет-мемы способом незаметной передачи информации"
STATEMENT_TEMPLATE = '''
Специалисты в области компьютерных расследований из компании Rubelfzederg
Security представили доклад, в котором говорится о роли интернет-мемов в
кибербезопасности. Проведённое исследование показало, что зачастую публикуемые
для развлечения картинки содержат скрытую информацию, доступ к которой можно
получить, только зная метод, использованный передающим лицом. В качестве
примера сотрудники Rubelfzederg Security приводят давно ставшее популярным
[изображение](/static/files/yykjklijlxtwmxe/{}) нескольких десятков котов и
собак. «Новейшие технологии, используемые в том числе для создания искусственного
интеллекта, позволили нам сделать однозначный вывод о наличии скрытой
информации в этой не вызывающей подозрений картинке», — говорится в докладе.
Отметим, однако, что эксперты не смогли извлечь информацию, которая, по их
мнению, содержится в изображении. «Извлечение этой информации — тема для
будущих исследований», — заявил специалист Rubelfzederg Марк Заслански.
'''
